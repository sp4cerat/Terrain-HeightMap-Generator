...
int ogl_tex_new(unsigned int size_x, unsigned int size_y, int filter=GL_LINEAR,int repeat=GL_CLAMP_TO_EDGE,int type1=GL_RGBA,int type2=GL_RGBA,unsigned char* data=0,int type3=GL_UNSIGNED_BYTE)
{
	if(!data) error_stop("ogl_tex_new data==0");
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, (filter==GL_LINEAR_MIPMAP_LINEAR) ? GL_LINEAR : filter );ogl_check_error();
...
{
	if(bmp.bpp==32)
	return ogl_tex_new(bmp.width, bmp.height,filter,repeat, GL_RGBA, GL_RGBA,bmp.data);

	return ogl_tex_new(bmp.width, bmp.height,filter,repeat, GL_RGB, GL_RGB,bmp.data);
}

int ogl_tex_float_bmp(const Bmp &bmp, int filter=GL_LINEAR,int repeat=GL_CLAMP_TO_EDGE)
{
	if(bmp.bpp!=32)return -1;
	return ogl_tex_new(bmp.width, bmp.height,filter,repeat, GL_LUMINANCE16F_ARB, GL_LUMINANCE,bmp.data, GL_FLOAT);
}
